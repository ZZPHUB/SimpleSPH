...
__global__ void sph_L_rho(SPH_CUDA *cuda,SPH_ARG *arg,SPH_RIGID *rigid)
{
    ...
    if( threadIdx.x < cuda->pair_count[mesh_id])
    {
        ...
        tmp_Lrho_x_i = (cuda->Lxx[index_i]*cuda->dwdx[id] + cuda->Lxy[index_i]*cuda->dwdy[id])*arg->m/cuda->rho[index_j];
        tmp_Lrho_y_i = (cuda->Lyx[index_i]*cuda->dwdx[id] + cuda->Lyy[index_i]*cuda->dwdy[id])*arg->m/cuda->rho[index_j];
        atomicAdd(&(cuda->Lrho_x[index_i]),tmp_Lrho_x_i);
        atomicAdd(&(cuda->Lrho_y[index_i]),tmp_Lrho_y_i);

        tmp_Lrho_x_j = (cuda->Lxx[index_j]*cuda->dwdx[id] + cuda->Lxy[index_j]*cuda->dwdy[id])*arg->m/cuda->rho[index_i];
        tmp_Lrho_y_j = (cuda->Lyx[index_j]*cuda->dwdx[id] + cuda->Lyy[index_j]*cuda->dwdy[id])*arg->m/cuda->rho[index_i];
