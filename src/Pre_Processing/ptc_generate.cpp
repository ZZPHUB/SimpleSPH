...
void fluid_ptc_generate(SPH_PARTICLE *particle)
{
    for(int i=0;i<(FLUID_LENGTH_NUM);i++)
    {
        ...
        {
            particle->x[i*(FLUID_DEEPTH_NUM)+j] = (i+3)*PTC_SPACING;
            particle->y[i*(FLUID_DEEPTH_NUM)+j] = (j+3)*PTC_SPACING;
...
{
    int k =0;
    for(int i=0;i<(FLUID_LENGTH_NUM+6);i++)
    {
        for(int j=0;j<(FLUID_DEEPTH_NUM+8);j++)
        {
            if(i<2 || i> FLUID_LENGTH_NUM+3)
            {
